...
import { BlockSymbol, ProfilePhoto, Vector, Scraps } from "./settingImages";
    ...
    <div className="settings_items setting_home_grid">
      <div className="mt-2 pt-4 text-center flex flex-col shadow-lg rounded-xl setting_profile_disp overflow-hidden">
        <div className="w-[130px] border-[3px] rounded-full border-[#ED2590] mx-auto">
          <img
            src={userData?.avatar || ProfilePhoto}
            alt="ProfilePhoto"
            className="w-full"
          />
        </div>
        <p className="font-bold text-[#4B4B4B] mt-4 leading-6">{`${
          userData?.firstName || "Yashie"
        } ${userData?.lastName || "Glory"}`}</p>
        <p className="text-sm text-[#4B4B4B] mt-2 leading-6">
          {`${userData?.nationality || "Nigeria"}`}
        </p>
        <div className="px-[30px] flex flex-col justify-end mb-20">
          <hr />
          <IconTextBox
            text={userData?.designation || "Designer"}
            icon={Vector}
          />
          {/* <IconTextBox text={userData?.someOtherField} icon={BlockSymbol} /> */}
          <hr />
          <IconTextBox
            text={userData?.email || "yashieglory@example.com"}
            icon={Scraps}
          />
        </div>
      </div>
